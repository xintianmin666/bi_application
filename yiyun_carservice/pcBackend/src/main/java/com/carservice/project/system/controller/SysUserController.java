package com.carservice.project.system.controller;
...
import com.carservice.project.system.service.ISysPostService;
import com.carservice.project.system.service.ISysRoleService;
...
public class SysUserController extends BaseController
{
    ...
    public AjaxResult addMerchant(@Validated @RequestBody SysUser user)
    {
        if (UserConstants.NOT_UNIQUE.equals(userService.checkUserNameUnique(user.getUserName())))
        {
            return AjaxResult.error("新增'" + user.getUserName() + "'失败，登录账号已存在");
        }
        if (UserConstants.NOT_UNIQUE.equals(userService.checkNickNameUnique(user.getNickName())))
        {
            return AjaxResult.error("新增商户名'" + user.getNickName() + "'失败，商户名已存在");
        }
        user.setCreateBy(SecurityUtils.getUsername());
        user.setPassword(SecurityUtils.encryptPassword(user.getPassword()));
        return toAjax(userService.insertMerchant(user));
    }

...
public class ValidateUtil {
  ...
  // 验证密码是否同时含有数字和字母
  public static boolean hasNumberCode(String value) {
    boolean isDigit = false; // 定义一个boolean值，用来表示是否包含数字
    boolean isLetter = false; // 定义一个boolean值，用来表示是否包含字母
    for (int i = 0; i < value.length(); i++) { // 循环遍历字符串
      if (Character.isDigit(value.charAt(i))) { // 用char包装类中的判断数字的方法判断每一个字符
        isDigit = true;
      }
      if (Character.isLetter(value.charAt(i))) { // 用char包装类中的判断字母的方法判断每一个字符
        isLetter = true;
      }
    }
  ...
  public static void main(String[] args) throws ParseException {
    ...
    String IDCardNum = "34052119760416564X";
    System.out.println(ValidateUtil.IDCardValidate(IDCardNum));
  }
}

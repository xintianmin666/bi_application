...
//支付回调
export const payWx = (e, url, orderId,rtPoints) => {
	 const payCallback = (url) => {
		 uni.requestPayment({
		 	...
		 	success: function(t) {
                ...
                uni.showToast({
		 		...
		 		rtPoints ? uni.navigateTo({
		 		    url: url + "?orderId=" + orderId+'&rtPoints='+rtPoints
		 		}) : uni.switchTab({
		 	...
		 	fail: function(t) {
               console.log(rtPoints)
		 		t.errMsg && "-1" != t.errMsg.indexOf("cancel") ? rtPoints ? uni.navigateTo({
		 		    url: url + "?orderId=" + orderId+'&rtPoints='+rtPoints
		 		}) : uni.switchTab({
		 		    url: url + "?orderId=" + orderId,
                    fail(res) {
